...
function generateQRcode() {
	# Generate a QR code for a WireGuard peer.
      ...
      echo "
      Для какого клиента сгенерировать QR-код?
      Надо написать словами!
      "
	...
	grep -E "^### Client" "/etc/wireguard/wg0.conf" | cut -d ' ' -f 3 | nl -s ') '
      # Prompt the user to enter the desired peer's name and store it in the VIEW_CLIENT_NAME variable
      read -rp"
      Напиши словами!: " VIEW_CLIENT_NAME
      # Check if the config file for the specified peer exists
      if [ -f "/root/wg0-client-${VIEW_CLIENT_NAME}.conf" ]; then
        # Generate a QR code for the specified peer's config file and display it in the terminal
		qrencode -t ansiutf8 -l L <"/root/wg0-client-${VIEW_CLIENT_NAME}.conf"
        # Print the file path of the specified peer's config file
        echo "
        О, царь! Вот Ваш QR-код! --> /root/wg0-client-${VIEW_CLIENT_NAME}.conf
        "
      else
        # If the config file for the specified peer does not exist, print an error message
        echo "
        Error: The peer you specified could not be found. Please ensure you've entered the correct information.
        "
        exit
      fi
}
...
function manageMenu() {
	...
	echo "   3) Generate the QR-code for an old client"
	echo "   4) Revoke existing user"
	echo "   5) Uninstall WireGuard"
	echo "   6) Exit"
	until [[ ${MENU_OPTION} =~ ^[1-6]$ ]]; do
		read -rp "
Select an option [1-6]: " MENU_OPTION
	done
	case "${MENU_OPTION}" in
	1)
		newClient
